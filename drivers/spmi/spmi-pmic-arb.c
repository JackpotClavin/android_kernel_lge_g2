...
periph_interrupt(struct spmi_pmic_arb_dev *pmic_arb, u8 apid)
{
	...
	enable = readl_relaxed(pmic_arb->intr + SPMI_PIC_ACC_ENABLE(apid));
	if (!(enable & 0x1)) {
		dev_err(pmic_arb->dev, "spurious interrupt: id:0x%x and apid:0x%x\n", ppid, apid);
		return IRQ_HANDLED;
	}

	if (!is_apid_valid(pmic_arb, apid)) {
		dev_err(pmic_arb->dev, "unknown peripheral: id:0x%x and apid:0x%x\n", ppid, apid);
		/* return IRQ_NONE; */
	}
