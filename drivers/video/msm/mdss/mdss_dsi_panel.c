...
#if defined(CONFIG_MACH_LGE) && !(defined(CONFIG_MACH_MSM8974_Z_KR) || defined(CONFIG_MACH_MSM8974_Z_US))
#include "mdss_mdp.h"
#include "mdss_fb.h"
...
static int img_tune_value = 1;	// Default value : cm0(standard) + plc 60%
...
int mdss_dsi_panel_img_tune_apply(unsigned int screen_mode)
{
	...
	if (pdata_base->panel_info.panel_power_on == 0) {
		pr_err("%s: panel off state. This screen mode will be applied next time.\n", __func__);
		return -EINVAL;
	}
	/*
	...
	*/
	...
	if (screen_mode == IMG_TUNE_COUNT) {
		pr_info("%s: send the screen mode on(%d) from kernel.\n", __func__, img_tune_value);
	} else {
		img_tune_value = screen_mode;
		pr_info("%s: send the screen mode on(%d) from user.\n", __func__, img_tune_value);
	}

	if(img_tune_cmds_set->img_tune_cmds[img_tune_value].cmd_cnt) {
		mdss_mdp_clk_ctrl(MDP_BLOCK_POWER_ON, false);
//		0 : CM0 / 1 : CM0 + PLC 60% / 2 : CM1 / 3 : CM1 + PLC 60% / 4 : CM2 / 5 : CM2 + PLC 60%
		mdss_dsi_panel_cmds_send(ctrl, &img_tune_cmds_set->img_tune_cmds[img_tune_value]);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%x", &tune_mode);
	mdss_dsi_panel_img_tune_apply(tune_mode);

	return ret;
}
DEVICE_ATTR(img_tune_mode, 0644, img_tune_mode_get, img_tune_mode_set);
#endif
...
#if defined(CONFIG_OLED_SUPPORT) && defined(CONFIG_LGE_OLED_IMG_TUNING)
	rc = device_create_file(&pdev->dev, &dev_attr_img_tune_mode);
	if(!rc)
		pr_info("%s: device file(img_tune_mode) create success!\n",__func__);
	else
		pr_err("%s: device file(img_tune_mode) create fail!\n",__func__);
#endif
