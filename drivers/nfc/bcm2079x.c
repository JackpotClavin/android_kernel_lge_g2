                     ...
                     unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_POWER_CTL:
        ...
        // LGE_START byunggu.kang@lge.com 2013-07-21 Modify for IRQ Exception Handle
        //dev_info(&bcm2079x_dev->client->dev,"%s, BCMNFC_POWER_CTL (%x, %lx):\n", __func__, cmd, arg);
        if ((sPowerState == NFC_POWER_ON) && (arg == 0)) {
            //dev_info(&bcm2079x_dev->client->dev,"%s, NFC_POWER_OFF\n", __func__);
            if (bcm2079x_dev->count_irq > 0) {
                dev_err(&bcm2079x_dev->client->dev,"%s, Clear IRQ\n", __func__);
                bcm2079x_dev->count_irq = 0;
                wake_unlock(&nfc_wake_lock);
            }
            ...
            //dev_info(&bcm2079x_dev->client->dev,"%s, NFC_POWER_ON\n", __func__);
