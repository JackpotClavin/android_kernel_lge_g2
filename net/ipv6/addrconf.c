...
// LGE_CHANGE_S, [LGE_DATA][LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER], heeyeon.nah@lge.com, 2013-05-21
#ifdef CONFIG_LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER
        printk(KERN_DEBUG "[LGE_DATA][%s()][AC_DAD] ipv6_addr_type == %d", __func__, ipv6AddrType);
        printk(KERN_DEBUG "[LGE_DATA][%s()] original when == %lu", __func__, when);

        if(ipv6AddrType == LGE_DATA_GLOBAL_SCOPE){
            printk(KERN_DEBUG "[LGE_DATA][%s()] ipv6AddrType == %d, global scope", __func__, ipv6AddrType);
        }else if(ipv6AddrType == LGE_DATA_LINK_LOCAL_SCOPE){
            printk(KERN_DEBUG "[LGE_DATA][%s()] ipv6AddrType == %d, link-local scope", __func__, ipv6AddrType);
        }

        if(ipv6AddrType == LGE_DATA_GLOBAL_SCOPE){
			printk(KERN_DEBUG "[LGE_DATA][%s()] The waiting time for Global DAD is set as zero in case of global IPv6 address !", __func__);
//If you want to send the DAD Packet for Global IPv6 address, the below line should be blocked.
            ifp->probes = 0;

//We can set the value of waiting time as zero for Global IPv6 address.
//            when = 0;
        }

        printk(KERN_DEBUG "[LGE_DATA][%s()] addrconf_dad_timer was called !", __func__);
#endif
// LGE_CHANGE_E, [LGE_DATA][LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER], heeyeon.nah@lge.com, 2013-05-21
...
// LGE_CHANGE_S, [LGE_DATA][LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER], heeyeon.nah@lge.com, 2013-05-21
#ifdef CONFIG_LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER
        printk(KERN_DEBUG "[LGE_DATA][%s()][AC_RS] ipv6_addr_type == %d, addrconf_rs_timer was called !", __func__, ipv6AddrType);
#endif
// LGE_CHANGE_E, [LGE_DATA][LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER], heeyeon.nah@lge.com, 2013-05-21
...
#ifdef CONFIG_LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER
    printk(KERN_DEBUG "[LGE_DATA][%s()] modified when == %lu\n", __func__, when);
#endif
...
#ifdef CONFIG_LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER
        printk(KERN_DEBUG "[LGE_DATA][%s()] RA was received!", __func__);
#endif
		goto out;
    }
...
// LGE_CHANGE_S, [LGE_DATA][LGP_DATA_TCPIP_SLAAC_IPV6_ALLOCATION_BOOSTER], heeyeon.nah@lge.com, 2013-05-21
    ...
    if(InterfaceNameToApply == NULL){
        printk(KERN_DEBUG "[LGE_DATA] InterfaceNameToApply is NULL !\n");
		return;
    }
    ...
    if(strcmp(InterfaceNameToApply, CurrentInterfaceName) == 0){//In case of rmnet, this patch will be applied bacause We should not impact to the Wi-Fi.
